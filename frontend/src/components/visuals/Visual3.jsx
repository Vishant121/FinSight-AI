...
const Visual3 = ({ transactions }) => {
  // Filter only saving transactions
  const savingData = transactions.filter(txn => txn.type === 'saving');

  // Group savings by category
  const dataByCategory = savingData.reduce((acc, txn) => {
  ...
  return (
    <div className="p-4 bg-[#e5e5e5] rounded-2xl shadow-md w-full md:w-[600px]">
      <h2 className="text-xl font-semibold mb-4 text-center">Saving Distribution by Category</h2>
